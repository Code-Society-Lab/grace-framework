...
By default, a database is scaffolded alongside your bot. Pass `--no-database` to skip it — you can always add one later with `grace generate database` (see [Database Management](database.md#adding-a-database-later)):
...
|---|---|
| `alembic.ini` | Alembic configuration file for migrations (only if a database was included) |
...
| `db/` | Database migrations, seeds, and Alembic environment (only if a database was included) |
